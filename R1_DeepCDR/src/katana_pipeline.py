...
from katana.ai.train import DistTrainer, Trainer
...
class RecipePipeline:
    ...
    def train(self, model_hp: DeepCDRConfig, training_hp: TrainingConfig) -> None:
        ...
        def remote_training(graph: Graph, model_hp, training_hp):
            torch.set_num_threads(training_hp.torch_num_threads)
    ...
    def test(self, training_hp: TrainingConfig) -> None:
        ...
        def remote_testing(graph: Graph, training_hp):
            ...
            res, pred = trainer.test(data_loader_test)
            return res, pred, df_test["label"]

        res, ypred, label = self.graph.run(lambda g: remote_testing(g, training_hp))
        return utils.compare_baselines(res), ypred, label

    @utils.disable_warnings
    def plot(self, ytrue, ypred) -> None:
        ...
        """
        utils.plot_prediction(ytrue, ypred)
