...
#  From, https://github.com/KatanaGraph/solutions/blob/main/fsi/src/katana_ai.py
...
def get_split(graph, split_id, split_name="split", node_label=None, node_label_prop="node_label"):
    ...
    if node_label is None:
        node_set = [
            ...
            if s == split_id and i < graph.num_master_nodes()
        ]
    else:
...
def evaluate_model(model, test_dataloader, remote_execution=False):
    ...
    all_x = []
    all_seeds = []
    #writer.add_text('Model Eval', f'Start Eval', 0)
    for batch in iter(test_dataloader):
        #batch.to(exec_device)
        #writer.add_text('Model Eval', f'Batch Type: {type(batch)}', 0)
        prediction, embed = model(batch)#.argmax(dim=1)
        #writer.add_text('Model Eval', f'Embed Size: {embed.shape}', 0)
        all_x.append(prediction[batch.seed_nodes])
    ...
    return acc, f1_score, precision, recall, all_embed, all_y, all_seeds, all_x
...
def train_model(model, loss_function, optimizer, writer, train_dataloader, test_dataloader, args):
    ...
    from katana.distributed.network import NetworkInterface
    ...
    for epoch in range(args.num_epochs):
        ...
        test_acc, test_f1_score, test_precision, test_recall, test_embed, test_y, _, _ = evaluate_model(model, test_dataloader)
        train_acc, train_f1_score, train_precision, train_recall, _, _, _, _ = evaluate_model(model, train_dataloader)
